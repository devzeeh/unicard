...
// AddMerchantHandler creates a new merchant and its corresponding owner user
func (h *Handler) AddMerchantHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		if errors.As(err, &validationErrs) {
			...
			msg := "Validation failed on field: " + firstErr.Field()
	...
	// Using business email as username for simplicity, you could use a dedicated username field
	username := req.BusinessEmail
	_, err = tx.Exec(userStmt, userID, username, req.OwnerName, req.BusinessEmail, req.BusinessPhone, string(hashedPassword), "merchant_admin", "active")
	if err != nil {
		tx.Rollback()
	...
	if err != nil {
		...
		jsonwrite.WriteJSON(w, http.StatusBadRequest, jsonwrite.APIResponse{
			Success: false,
			Message: "Failed to create merchant profile (registration num or email might exist)",
		})
		return
	}
	...
	if err := tx.Commit(); err != nil {
		...
		jsonwrite.WriteJSON(w, http.StatusInternalServerError, jsonwrite.APIResponse{
			Success: false,
			Message: "Failed to finalize creation",
		})
		return
	}
